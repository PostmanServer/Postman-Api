...
class Postman
{
     ...
     * PHP5 Constructor.
     *
     * Initializes a new Postman instance with key, secret , app ID and channel.
    ...
    public function __construct($auth_key, $secret, $app_id, $options = array(), $host = null, $port = null, $timeout = null)
    {
        ...
        if (isset($options['notification_host'])) {
            ...
            $this->settings['notification_host'] = 'notification.postman.com';
        }
        ...
        if (!array_key_exists('host', $this->settings)) {
            if (array_key_exists('host', $options)) {
                ...
                $this->settings['host'] = 'api-'.$options['cluster'].'.postman.com';
            } else {
                $this->settings['host'] = 'api.postman.com';
            }
        }
     ...
     *
     * @throws PostmanException if any required dependencies are missing
     *
     * @return void
     */
    private function check_compatibility()
    {
        if (!extension_loaded('curl')) {
            throw new PostmanException('The postman library requires the PHP cURL module. Please ensure it is installed');
        }

        if (!extension_loaded('json')) {
            throw new PostmanException('The postman library requires the PHP JSON module. Please ensure it is installed');
        }

        if (!in_array('sha256', hash_algos())) {
            throw new PostmanException('SHA256 appears to be unsupported - make sure you have support for it, or upgrade your version of PHP.');
        }
    }
     ...
     *
     * @throws PostmanException if $channels is too big or any channel is invalid
     *
     * @return void
     */
    private function validate_channels($channels)
    {
        if (count($channels) > 100) {
            throw new PostmanException('An event can be triggered on a maximum of 100 channels in a single call.');
        }
     ...
     *
     * @throws PostmanException if $channel is invalid
     *
     * @return void
     */
    private function validate_channel($channel)
    {
        if (!preg_match('/\A[-a-zA-Z0-9_=@,.;]+\z/', $channel)) {
            throw new PostmanException('Invalid channel name '.$channel);
        }
    }
     ...
     *
     * @throws PostmanException if $socket_id is invalid
     */
    private function validate_socket_id($socket_id)
    {
        if ($socket_id !== null && !preg_match('/\A\d+\.\d+\z/', $socket_id)) {
            throw new PostmanException('Invalid socket ID '.$socket_id);
        }
    }
    ...
    private function create_curl($domain, $s_url, $request_method = 'GET', $query_params = array())
    {
        ...
        if ($this->ch === false) {
            throw new PostmanException('Could not initialise cURL!');
        }
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, array(
            ...
            'Expect:',
            'X-Pusher-Library: postman-api '.self::$VERSION,
        ));
     ...
     * @param bool  $debug
     *
     * @throws PostmanException if validation fails.
     *
     * @return bool|string
     */
    public function notify($interests, $data = array(), $debug = false)
    {
        ...
        if (count($interests) === 0) {
            throw new PostmanException('$interests array must not be empty');
        }
